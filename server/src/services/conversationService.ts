...
export const sendMessage = async (
  ...
  const userMessage = await Message.create({ conversationId, role: 'user', content });
  ...
  const aiMessage = await Message.create({ conversationId, role: 'assistant', content: aiResponse });

  await Conversation.findByIdAndUpdate(conversationId, { updatedAt: new Date() });

  if (conversation.title === 'New Conversation') {
    await Conversation.findByIdAndUpdate(conversationId, { title: content.slice(0, 50) });
  }
